...
function createAnswerPlan(questions: Question[], correctCount: number): DevScenarioPlan {
  return {
    answers: questions.map((question, index) => index < correctCount ? question.correctIndex : guaranteedWrongAnswer(question)),
  };
}
...
export function buildScenarioAnswers(
  ...
  if (context.paper === 'a' && context.module === 'm1') {
    if (scenario === 'pass-with-incorrect') {
      return disabled('Paper A Module 1 cannot demonstrate a final pass with incorrect answers because Module 2 contributes to the combined score.');
    }
    ...
    if (scenario === 'pass-with-incorrect') {
      ...
      if (correctCount >= total) {
        return disabled('This pass scenario requires every Module 2 answer to be correct, so it cannot include an incorrect answer.');
      }
    } else {
      correctCount = paper.passMark - context.previousModuleScore - 1;
      if (correctCount < 0) {
        return disabled('A failing Paper A result is impossible because the recorded Module 1 score already reaches the paper pass mark.');
      }
    }
  } else {
