...
export default function ReviewClient({ paper, moduleKey, seed, questions, attemptId }: Props) {
  ...
  const finishAttempt = useCallback(
    (mode: "manual" | "auto") => {
      ...
      if (current?.status !== "in-progress" || submittingRef.current) {
        return;
      }
          ...
          return (
            ...
            >
              <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                ...
                <span
                  className={clsx(
                    "inline-flex shrink-0 items-center gap-2 whitespace-nowrap text-xs font-bold uppercase tracking-wide",
                    isAnswered ? "text-success" : "text-danger",
                  )}
                >
                  {isAnswered ? (
              ...
              <button
                ...
                type="button"
                aria-label={`Edit answer for question ${String(index + 1).padStart(2, "0")}`}
                onClick={() =>
                  router.push(
                    `${basePath}?attempt=${encodeURIComponent(attempt.attemptId)}&question=${index}&return=review`,
                  )
                }
              >
