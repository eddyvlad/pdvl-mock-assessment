...
export default function PracticeClient({
  ...
  const finishAttempt = useCallback(
    (record: AttemptRecordV2, mode: "manual" | "auto") => {
  ...
  useEffect(() => {
    ...
    if (stored && stored.status === "in-progress" && isResumableAttempt(stored)) {
      ...
      const restored =
        savedQuestion === stored.currentQuestion
          ? stored
          : {
              ...stored,
    ...
    const fresh = createAttemptRecord({
      paper,
      module: moduleKey,
      seed,
      total,
    ...
    <main className="page-shell max-w-5xl">
      ...
      <section className="card border-primary/30 p-5 sm:p-8" aria-labelledby={`question-${currentQuestion}`}>
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <h2
              ...
              tabIndex={-1}
              className="practice-question-heading mb-0 max-w-3xl font-sans text-2xl font-bold leading-8 tracking-normal sm:text-3xl"
            >
              {question.prompt}
            </h2>
          </div>
        ...
        <div className="mt-8 flex flex-wrap justify-between gap-3 border-t border-border pt-5">
          ...
          >
            {currentQuestion === total - 1 ? "Review answers" : "Next question"}{" "}
