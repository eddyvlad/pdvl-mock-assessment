...
Added the missing-seed route for valid paper and module combinations. It generates a six-character base62 seed and redirects to the canonical seeded practice URL, while invalid paper or module combinations continue to return not found.

Browser verification confirmed that `/practice/a/m1` redirects to a valid seeded route and loads the first question. Validation passed with lint, typecheck, tests, build, and `git diff --check`.
