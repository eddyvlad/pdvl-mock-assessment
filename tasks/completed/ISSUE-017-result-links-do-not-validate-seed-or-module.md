...
Result, review, and practice routes now validate an explicit attempt ID against the route's paper, module, and seed before rendering saved answers. Mismatched links fail closed with a recovery state, and Paper A Module 2 results require the matching submitted Module 1 attempt before showing a combined score.

Added README documentation for route-bound attempt IDs. Browser verification confirmed mismatched links are rejected in all three attempt-bearing views. Validation passed with lint, typecheck, tests, build, and `git diff --check`.
